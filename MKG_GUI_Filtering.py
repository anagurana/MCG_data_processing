...
def create_filtration_window(data):
    ...
    create_apply_button(parFrames[1], data, fsEntry, passbandEntry, checkValues, dictionary, figures, plots)
    # think about it
    modify_plots(data, fsEntry, passbandEntry, checkValues, dictionary, figures, plots)
...
def create_entries(parFrames):
    ...
    Label(parFrames[1], text="Uśrednienie dla                 cykli serca", font='20').place(relx=0.02, rely=0.45,
                                                                                            relwidth=0.96,
                                                                                            relheight=0.1)
    heartBeats = Entry(parFrames[1], state="readonly").place(relx=0.5, rely=0.45, relwidth=0.1, relheight=0.1)
...
def create_apply_button(frame, data, fsEntry, passbandEntry, checkValues, dictionary, figures, plots):
    applyB = Button(frame, text="Zastosuj", font=15, command=lambda: modify_plots(data, fsEntry, passbandEntry, checkValues, dictionary, figures, plots))
...
def modify_data(data, Fs, passbandEntry, checkValues, dictionary):
    ...
    return data
def modify_plots(data, fsEntry, passbandEntry, checkValues, dictionary, figures, plots):
    Fs = int(fsEntry.get())
    modified_data = modify_data(data, Fs, passbandEntry, checkValues, dictionary)
    draw_signal(modified_data, Fs, figures[0], plots[0])
...
def draw_signal(data, Fs, mainFigure, drawn_plot):
